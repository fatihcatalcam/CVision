...
export function VerifyEmailPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(Array.isArray(detail) ? detail.map((d: any) => d.msg).join(', ') : detail || 'Verification failed. Please try again.');
      setDigits(['', '', '', '', '']);
      inputRefs.current[0]?.focus();
    } finally {
      setIsLoading(false);
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const detail = error.response?.data?.detail;
      toast.error(detail || 'Failed to resend code. Please try again.');
    } finally {
