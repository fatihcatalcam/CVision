...
import {
  ArrowLeft, User, Lock, Shield, Eye, EyeOff,
  Check, X, Loader2, Coins, Calendar, Mail, Sun, Bell,
...
export function SettingsPage() {
  ...
  // Credits: the buy entry point only appears once packs are actually on sale.
  const { onSale: packsOnSale } = useCreditPacks();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <Card className="p-4 space-y-3">
            ...
            {/* Was a "used / weekly quota" progress bar. There is no quota to
                fill any more - a balance is spent at different rates by
                different actions, so the bar was measuring nothing. */}
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-[#6B6A65] dark:text-[#908d89]">{t('credits.label')}</span>
                <span className="text-[#111111] dark:text-[#e8e7e4] font-bold stat-number">
                  {user?.credits ?? 0}
                </span>
              </div>
              <p className="text-[10px] text-[#6B6A65] dark:text-[#908d89] leading-relaxed">
                {t('credits.weekly', { weekly: WEEKLY_CREDITS })}
              </p>
              <div className="flex justify-between text-sm pt-1">
                <span className="text-[#6B6A65] dark:text-[#908d89]">{t('settings.analysesDone')}</span>
                <span className="text-[#111111] dark:text-[#e8e7e4] font-bold stat-number">{used}</span>
              </div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {/* 5 - Credits.
              Replaces the Subscription section. plan_type stopped gating
              anything when credits arrived, so a card announcing "Free plan -
              3 analyses per week" next to an "Upgrade to Pro" button was
              describing a product that no longer exists. What someone actually
              needs here is the balance and what each thing costs. */}
          <div className="surface p-6">
            <SectionHeader icon={CreditCard} title={t('settings.credits.title')} desc={t('settings.credits.subtitle')} />
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 rounded-xl bg-[#F7F6F3] dark:bg-white/[0.04] border border-[#EAEAEA] dark:border-white/[0.07]">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-[#FBF3DB] dark:bg-[#956400]/20 text-[#956400]">
                    <Coins className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-[#111111] dark:text-[#e8e7e4]">
                      {t('credits.cost', { cost: user?.credits ?? 0 })}
                    </p>
                ...
                {packsOnSale && (
                  <button onClick={() => navigate('/pricing')}
                    ...
                    {t('credits.buyCta')} <ChevronRight className="w-3 h-3" />
                  </button>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                {[
                  ...
                  <div key={label} className="flex justify-between text-xs">
                    <span className="text-[#6B6A65] dark:text-[#908d89]">{label}</span>
