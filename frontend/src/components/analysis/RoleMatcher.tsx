  ...
  return (
    <div className="space-y-4">
      {sortedRoles.map((rec, index) => {
        ...
        return (
          <div key={`${rec.role_title}-${index}`} className="flex flex-col gap-2 p-4 rounded-xl bg-[#F7F6F3] border border-[var(--color-card-border)]">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <Briefcase className="w-4 h-4 text-[var(--color-primary)]" />
                <h4 className="font-semibold text-[#111111]">{rec.role_title}</h4>
              </div>
              <span className="text-sm font-bold text-[#111111]">{scorePercentage}% Match</span>
            </div>
